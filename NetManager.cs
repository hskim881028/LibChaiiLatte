...
namespace LibChaiiLatte {
    ...
    public class NetManager : INetSocketListener, IEnumerable<NetPeer> {
        ...
        /// IPv6 support
        /// </summary>
        public const IPv6Mode IPv6Enabled = IPv6Mode.SeparateSocket;
        ...
        public NetManager(INetEventListener listener, bool autoRecycle, PacketLayerBase extraPacketLayer = null) {
        ...
        private void DisconnectPeer(NetPeer peer,
            ...
            CreateEvent(NetEvent.EType.Disconnect, peer, errorCode: socketErrorCode, disconnectReason: reason,
                        readerSource: eventData);
        }

        private void CreateEvent(NetEvent.EType type,
                                 NetPeer peer = null,
            ...
            switch (evt.Type) {
                ...
                case NetEvent.EType.Disconnect:
                    var info = new DisconnectInfo {
                        Reason = evt.DisconnectReason, AdditionalData = evt.DataReader, SocketErrorCode = evt.ErrorCode
                    };
        ...
        void INetSocketListener.OnMessageReceived(byte[] data,
                                                  int length,
        ...
        private void ProcessConnectRequest(IPEndPoint remoteEndPoint,
                                           NetPeer netPeer,
            ...
            if (netPeer != null) {
                ...
                NetDebug.Write("ConnectRequest LastId: {0}, NewId: {1}, EP: {2}, Result: {3}", netPeer.ConnectTime,
                               connRequest.ConnectionTime, remoteEndPoint, processResult);
            ...
            lock (_requestsDict) {
                ...
                req = new ConnectionRequest(connRequest.ConnectionTime, connectionNumber, connRequest.Data,
                                            remoteEndPoint, this);
                    ...
                    if (peerFound) {
                        ...
                        DisconnectPeerForce(netPeer,
                                            disconnectResult == DisconnectResult.Disconnect
                                                ...
                                                : DisconnectReason.ConnectionRejected, 0, packet);
                    }
                    else {
                        NetPacketPool.Recycle(packet);
                    }
