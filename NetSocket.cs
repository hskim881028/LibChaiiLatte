        ...
        private void OnApplicationPause(bool pause) {
            ...
            if (pause) {
                ...
                if (!Socket.Bind(BindAddrIPv4, BindAddrIPv6, Port, Reuse, IPv6)) {
                    NetDebug.WriteError("[S] Cannot restore connection \"{0}\",\"{1}\" port {2}", BindAddrIPv4,
                                        BindAddrIPv6, Port);
...
#pragma warning restore 618
                ...
                try {
                    ...
                    switch (ex.SocketErrorCode) {
                        ...
                        case SocketError.TimedOut:
                            NetDebug.Write(NetLogLevel.Trace, "[R]Ignored error: {0} - {1}", (int) ex.SocketErrorCode,
                                           ex.ToString());
                            break;
                        default:
        ...
        public bool SendBroadcast(byte[] data, int offset, int size, int port) {
            ...
            try {
                broadcastSuccess = _udpSocketv4.SendTo(data, offset, size, SocketFlags.None,
                                                       new IPEndPoint(IPAddress.Broadcast, port)) > 0;

                if (_udpSocketv6 != null) {
                    multicastSuccess = _udpSocketv6.SendTo(data, offset, size, SocketFlags.None,
